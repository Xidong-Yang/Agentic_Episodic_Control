    ...
    def _gen_grid(self, width, height):
        ...
        while True:
            try:
                ...
                if self.assert_first and not self.instrs.surface(self).startswith(self.action_kinds[0]):
...
class LevelGen(RoomGridLevel):
    ...
    def gen_mission(self):
        if self._rand_float(0, 1) < self.locked_room_prob and (self.num_rows > 1 or self.num_cols > 1):  # need at least two rooms to have a locked room
...
def test():
    for idx, level_name in enumerate(level_dict.keys()):
        ...
        for i in range(0, 15):
            ...
            while True:
                action = rng.integers(0, mission.action_space.n - 1)
