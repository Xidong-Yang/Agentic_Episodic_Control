...
# Agent cell background color (can be changed)
AGENT_CELL_COLOR = np.array([173, 216, 230])  # Light blue for agent's cell
...
class Grid:
    ...
    def render(
        ...
        """
        Render this grid at a given scale and also return the map without agent or highlights
        ...
        map_img = np.zeros(shape=(height_px, width_px, 3), dtype=np.uint8)
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                # Render each tile without agent or highlight for the map
                map_tile_img = Grid.render_tile(
                    cell,
                    agent_dir=agent_dir if agent_here else None,
                    highlight=False,
                    tile_size=tile_size
                ...
                map_img[ymin:ymax, xmin:xmax, :] = map_tile_img
        
        # Create a new image for the agent-highlighted grid
        agent_highlighted_img = img.copy()

        # Fill the agent's cell with a specific color (e.g., red)
        if agent_pos is not None:
            agent_ymin = agent_pos[1] * tile_size
            agent_ymax = (agent_pos[1] + 1) * tile_size
            agent_xmin = agent_pos[0] * tile_size
            agent_xmax = (agent_pos[0] + 1) * tile_size
            agent_highlighted_img[agent_ymin:agent_ymax, agent_xmin:agent_xmax, :] = [173, 216, 230]  # Light blue color
            # agent_highlighted_img[agent_ymin:agent_ymax, agent_xmin:agent_xmax, 3] = 128  # Set alpha channel for transparency

        return img, map_img, agent_pos  # Return the new image
    ...
    def render(self, mode='human', close=False, highlight=True, tile_size=TILE_PIXELS):
        ...
        for vis_j in range(0, self.agent_view_size):
            for vis_i in range(0, self.agent_view_size):
                ...
                # import pdb; pdb.set_trace()
        # print('========highlight_mask========', highlight_mask)
        # Render the whole grid
        img, map_img, agent_highlighted_img = self.grid.render(
            tile_size,
            self.agent_pos,
            self.agent_dir,
        ...
        return img, map_img, highlight_mask, agent_highlighted_img
